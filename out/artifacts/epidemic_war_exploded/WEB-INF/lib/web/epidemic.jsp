...
<script type="text/javascript">
    $(function () {
        //页面载入完成后会自动执行该函数
        //定义个用来给表格中装载数据的函数
        //epidemics为形参 传入参数为调用函数时传入的值
        var fillToTable = function (epidemics) {
            //此时epidemics为resp.data
            //{pageContext.request.contextPath}/epidemicData/ajax/lastestData从该路径返回的值
            var tbody1 = $("#tbody1");
            tbody1.empty();
        ...
        var xinqi = date.getDay() + 1;
        var dateStr = year + '\t年\t' + month + '\t月\t' + day + "\t日\t" + "星期\t" + xinqi;
        var lengendDate = year + '\t年\t' + month + '\t月\t' + day + "\t日\t";
        ...
        //发送请求获取最新疫情数据  resp是后端返回的数据  回调函数 传递参数为空{}
